...
function setup() {
    ...
    for (var i = 0; i < robots.length; i++) {
        var robot = robots[i];
        console.log(robot.firstName + " " + robot.secondName);
    }

    // 2. Find robot with surname "Katan" and highlight in green
    for (var i = 0; i < robots.length; i++) {
        var robot = robots[i];
        if (robot.secondName === "Katan") {
            console.log("Found Katan: " + robot.firstName + " " + robot.secondName);
            highlight(robot, 0, 255, 0);
            break;
        }
    }

    // 3. Select the robot with the highest IQ and highlight it in blue. Also, print its name to the console
    var highestIQ = 0;
    var smartestRobot = null;
    for (var i = 0; i < robots.length; i++) {
        var robot = robots[i];
        if (robot.iq > highestIQ) {
            highestIQ = robot.iq;
            smartestRobot = robot;
        }
    }
    if (smartestRobot) {
        console.log("Smartest Robot: " + smartestRobot.firstName + " " + smartestRobot.secondName + " with IQ: " + smartestRobot.iq);
        highlight(smartestRobot, 0, 0, 255);
    }

    // 4. Select the robot with the smallest serial number and highlight it in purple. Also, print its name to the console
    var oldestSerialNumber = Infinity;
    var oldestRobot = null;
    for (var i = 0; i < robots.length; i++) {
        var robot = robots[i];
        if (robot.serialNumber < oldestSerialNumber) {
            oldestSerialNumber = robot.serialNumber;
            oldestRobot = robot;
        }
    }
    if (oldestRobot) {
        console.log("Oldest Robot: " + oldestRobot.firstName + " " + oldestRobot.secondName + " with Serial Number: " + oldestRobot.serialNumber);
        highlight(oldestRobot, 128, 0, 128);
    }

    // 5. Find the robot who wants to "destroy humanity" and highlight it in red. Also, print its name to the console
    for (var i = 0; i < robots.length; i++) {
        var robot = robots[i];
        if (robot.abilities.includes("destroy humanity")) {
            console.log("Rogue Robot: " + robot.firstName + " " + robot.secondName);
            highlight(robot, 255, 0, 0);
            break;
        }
    }

    // 6. Find any robot whose face is narrower than 70 and highlight it in yellow. Also, print its name to the console
    for (var i = 0; i < robots.length; i++) {
        var robot = robots[i];
        if (robot.physical.headWidth < 70) {
            console.log("Narrow Head Robot: " + robot.firstName + " " + robot.secondName + " with Head Width: " + robot.physical.headWidth);
            highlight(robot, 255, 255, 0);
        }
	}
}
